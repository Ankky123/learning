...
var orderItemCollection *mongo.Collection = database.OpenCollection(database.Client, "orerItem")
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
	//aggregation is very powerful tool, learn it
	...
	groupStage := bson.D{{"$group", bson.D{{"_id", bson.D{{"order_id", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}}}, {"payment_due", bson.D{{"$sum", "$amount"}}}, {"total_count", bson.D{{"$sum", 1}}}, {"order_items", bson.D{{"$push", "$$ROOT"}}}}}} //part 13
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while listing singular order item"})
			return
		}
